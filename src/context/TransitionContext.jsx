...
function LogoReveal({ snapshot, onDone }) {
  ...
  return createPortal(
    <>
      {/* Old-page snapshot: visible outside the growing circle */}
      <div
        ref={mountSnapshot}
        style={{
          ...
          pointerEvents: 'none',
          overflow: 'hidden',
          // transform creates a containing block for position:fixed children in the clone
          transform: 'scale(1)',
      ...
      {/* Logo: spins and grows in sync with the reveal hole */}
      <div style={{
...
export function TransitionProvider({ children }) {
  ...
  const [overlay, setOverlay] = useState(null) // { snapshot }

  const triggerTransition = useCallback((path, _x, _y, state) => {
    const snapshot = takeSnapshot()
    // Scroll to top so the new page initialises at scrollY=0.
    // The snapshot overlay covers this jump — the browser won't repaint until
    // after the overlay is mounted, so the user never sees the page jump.
  ...
  return (
    <TransitionContext.Provider value={{ triggerTransition, active: !!overlay }}>
      {children}
